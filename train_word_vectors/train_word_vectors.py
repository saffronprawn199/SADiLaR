    ...
    def __iter__(self):
        ...
        for row in self.df['Sentences']:
            try:
                ...
                sentence_stream = list(filter((' ').__ne__, sentence_stream))
            except Exception:
...
class TrainWordVectors(object):
    ...
    def trainWordVector(
        ...
        if self.typeEmbedding in 'fastText':
            ...
            '''Create directory'''
            if not os.path.isdir(word2vecModelPath):
                os.mkdir(word2vecModelPath)
                print("Directory ", word2vecModelPath, "Created ")
            else:
                print("Directory ", word2vecModelPath, "Already Exists ")

            self.trainWord2vec(word2vecModelPath, nameEmbedding)
    ...
    def trainWord2vec(self, wordvec_model_path, name_embedding):
        ...
        if self.use_pretrained:
            ...
            w2v_model.wv.intersect_word2vec_format('/Users/rhynostrydom/PycharmProjects/SADiLaR/Embeddings models/GoogleNews-vectors-negative300.bin.gz', binary=True)
        ...
        w2v_model.save(wordvec_model_path + name_embedding)

        print('Time to train the model: {} mins'.format(round((time() - t) / 60, 2)))

    def trainFastText(self, wordvec_model_path, name_embedding):
        ...
        save_path = os.path.join(wordvec_model_path, "{}".format(name_embedding))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--usePretrained',
                        metavar='bool',
                        type=bool,
                        default=False,
                        help="use if you want to initialize your word vectors on pretrained embeddings\
                            - default value is \"False\"")
    ...
    for key, value in skipGram.items():
        for typeEmbed in args.typeEmbedding:
            word_vectors.trainWordVector(embeddingDimension=args.embeddingDimension,
                                         epochs=args.epochs,
                                         sg=value,
                                         embMethod=key,
