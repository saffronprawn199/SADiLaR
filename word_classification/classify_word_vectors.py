...
def clean_text(text):
    ...
    text = ' '.join(word for word in text.split() if word not in stopwords_set)  # delete stopwords from text
    return text
...
def print_report(y_pred, y_test, y_pred_prob, my_tags, name, output_file):
    ...
    file.write('AUC %s' % roc_auc_score(y_test, y_pred_prob,  multi_class="ovr", average="micro") + '\n')
